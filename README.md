...
```env
PORT=3000
MONGODB_URI=mongodb://localhost:27017/testimonials_db?replicaSet=rs0
JWT_SECRET=your_super_secret_key_here
JWT_EXPIRY=7d
```

> **Важно:** Для корректной работы регистрации (POST /api/auth/register) требуется MongoDB **replica set**. Транзакции в MongoDB работают только на replica set / mongos, а не на одиночном mongod. Если вы используете локальный MongoDB, запустите его с replica set:
>
> ```bash
> # Инициализация replica set (один раз)
> mongosh --eval "rs.initiate()"
>
> # Или используйте MongoDB Compass с включенным replica set
> ```
...
Для корректной работы rate limiting за reverse proxy или балансировщиком, приложение использует `trust proxy` setting.
...
- **Для регистрации требуется MongoDB replica set** — транзакции работают только на replica set / mongos
